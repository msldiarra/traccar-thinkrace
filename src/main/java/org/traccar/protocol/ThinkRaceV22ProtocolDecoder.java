...
public class ThinkRaceV22ProtocolDecoder extends BaseProtocolDecoder {
    ...
    private static final Pattern PATTERN = new PatternBuilder()
            ...
            .groupBegin()
            .text(",")
            .expression("([^#]+)")               // WiFi data (tout jusqu'au #)
            .groupEnd("?")
            /* Original wifi code */
            /* .groupBegin()
            .text(",")
    ...
    protected Object decode(
        ...
        if (type.equals("CP01")) {
        ...
        } else if (type.equals("AP01") || type.equals("YP03") || type.equals("YP14")) {
            ...
            // Réutilise la dernière localisation connue (GPS ou LBS)
            getLastLocation(position, null);

            String[] parts = sentence.split(",");

            if (parts.length > 5) {
                String alarm = parts[5];
                if ("01".equals(alarm)) {
                    position.addAlarm(Position.ALARM_SOS);
                } else if ("05".equals(alarm) || "06".equals(alarm)) {
            ...
            try {
                ...
                int lac = Integer.parseInt(parts[3]);
